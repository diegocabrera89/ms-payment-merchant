...
// CreateMerchantHandler handler for createMerchantHandler new merchant.
func (h *MerchantHandler) CreateMerchantHandler(ctx context.Context, request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	logs.LogTrackingInfo("CreateMerchantHandler", ctx, request)
	createMerchantHandler, errorMerchantHandler := h.merchantService.CreateMerchantService(ctx, request)
	if errorMerchantHandler != nil {
		logs.LogTrackingError("CreateMerchantHandler", "CreateMerchantService", ctx, request, errorMerchantHandler)
		return response.ErrorResponse(http.StatusInternalServerError, constantsmicro.ErrorCreatingMerchant)
	}
...
func main() {
	...
	handlerWithLogging := metadata.MiddlewareMetadata(merchantHandler.CreateMerchantHandler)
